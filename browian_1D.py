...
def get_F_fnc(mode, F_scale=1, Ncl_scale=1, \
	...
	F0_data = F_data - np.log(D_fnc(Ncl))
	d_F0_data = np.sqrt(d_F_data**2 + (d_D_fnc(Ncl) / D_fnc(Ncl))**2)
	...
	F0_data = F0_data[ok_inds]
	d_F0_data = d_F0_data[ok_inds]
	
	if(mode == 'lin'):
		F_fnc = scipy.interpolate.interp1d(Ncl, F_data, bounds_error=False, fill_value='extrapolate')
		F0_fnc = scipy.interpolate.interp1d(Ncl, F0_data, bounds_error=False, fill_value='extrapolate')
		F0_grad_fnc = lambda n, dn=0.01, f=F0_fnc: (f(n + dn) - f(n - dn)) / (2 * dn)
	elif(mode == 'cube'):
		F_fnc = scipy.interpolate.CubicSpline(Ncl, F_data)
		F0_fnc = scipy.interpolate.CubicSpline(Ncl, F0_data)
		F0_grad_fnc = lambda n, f=F0_fnc: f(n, 1)
	
	return F_fnc, F0_fnc, F0_grad_fnc, Ncl, F_data, d_F_data,  F0_data, d_F0_data
...
def get_dNcl(Ncl, dt, D_fnc, dF0_fnc):
	D = D_fnc(Ncl)
	...
	dNcl = dF0_fnc(Ncl) * (- D * dt) + np.sqrt(2 * D * dt) * gauss_rands[rand_time]
...
def run_dynamics(Ncl_init, dt, Nt, D_fnc, dF0_fnc, \
	...
	for it in range(Nt):
		...
		times[it] = dt_use
		dNcl = get_dNcl(Ncl, dt_use, D_fnc, dF0_fnc)
...
def main():
	...
	# python browian_1D.py --D_mode surf --mode FPE_rate_est --Ncl_min 15
	...
	if(clargs.F_mode == 'data'):
		...
		F0_fnc = np.vectorize(lambda x, xmin=min(Ncl_interp), xmax=max(Ncl_interp), wmin=2, wmax=2: \
					(30 * ((x - (xmin + wmin))/wmin)**2 if(x < xmin + wmin) else (30*((x - (xmax - wmax))/wmax)**2 if(x > xmax - wmax) else 0))\
				)
		
		F0_grad_fnc = lambda x, dx=1e-2, f=F_fnc: (f(x + dx) - f(x - dx)) / (2 * dx)
	...
	if(mode in ['FPE_rate_est', 'FPE_rate_est_exact']):
		...
		rho_crit_ratio = np.sum(rho_precrit_Ncl * np.exp(F_fnc(Ncl_top) - F_fnc(rho_precrit_Ncl)))
		...
		ln_k_CNT = np.log(rho_crit * clargs.Dcrit_est[0] * clargs.Zfactor_est[0])
		...
		F0_interp_a = (F0_data_interp[1:] - F0_data_interp[:-1]) / (Ncl_interp[1:] - Ncl_interp[:-1])
		F0_interp_b = (Ncl_interp[1:] * F0_data_interp[:-1] - Ncl_interp[:-1] * F0_data_interp[1:]) / (Ncl_interp[1:] - Ncl_interp[:-1])
		...
		Ncl_est = np.linspace(Ncl_min, Ncl_B, 1000)
		A_integr1 = np.trapz(np.exp(-F0_fnc(Ncl_est))/D_fnc(Ncl_est), x=Ncl_est) / Z
		A_integr3 = np.trapz(np.exp(-F_fnc(Ncl_est)), x=Ncl_est) / Z
		if(D_mode == 'const'):
			A_integr = np.sum(np.exp(-F0_interp_b[Ncl_min_ind : Ncl_B_ind]) * (np.exp(-F0_interp_a[Ncl_min_ind : Ncl_B_ind] * Ncl_interp[Ncl_min_ind+1 : Ncl_B_ind+1]) - np.exp(-F0_interp_a[Ncl_min_ind : Ncl_B_ind] * Ncl_interp[Ncl_min_ind : Ncl_B_ind])) / (-F0_interp_a[Ncl_min_ind : Ncl_B_ind])) / Z / D_fnc(1.0)
		elif(D_mode == 'surf'):
			A_integr_fnc = np.vectorize(lambda x, a: scipy.special.erf(x) if(a < 0) else scipy.special.erfi(x))
			A_integr = np.sum(np.exp(-F0_interp_b[Ncl_min_ind : Ncl_B_ind]) / np.sqrt(np.abs(F0_interp_a[Ncl_min_ind : Ncl_B_ind])) * \
							(A_integr_fnc(np.sqrt(np.abs(F0_interp_a[Ncl_min_ind : Ncl_B_ind]) * Ncl_interp[Ncl_min_ind+1 : Ncl_B_ind+1]), -F0_interp_a[Ncl_min_ind : Ncl_B_ind]) - \
							A_integr_fnc(np.sqrt(np.abs(F0_interp_a[Ncl_min_ind : Ncl_B_ind]) * Ncl_interp[Ncl_min_ind : Ncl_B_ind]), -F0_interp_a[Ncl_min_ind : Ncl_B_ind]))) \
						* (np.sqrt(clargs.Ncl0 * np.pi) / clargs.D0 / Z)
		#A_integr2 = scipy.integrate.quad(lambda z, U_fnc=F0_fnc, D_fnc=D_fnc: (np.exp(-U_fnc(z))/D_fnc(z)), Ncl_min, Ncl_B)[0] / Z
		A_integr2 = scipy.integrate.quad(lambda z, U_fnc=F_fnc: (np.exp(-U_fnc(z))), Ncl_min, Ncl_B)[0] / Z
		
		print('A:', A_integr, A_integr1/A_integr, A_integr2/A_integr, A_integr3/A_integr)
		
		TP_integr2 = scipy.integrate.quad(lambda y, U_fnc=F0_fnc: np.exp(U_fnc(y)), Ncl_init, Ncl_B)[0] * Z
		Ncl_est = np.linspace(Ncl_init, Ncl_B, 1000)
		TP_integr1 = np.trapz(np.exp(F0_fnc(Ncl_est)), x=Ncl_est) * Z
		TP_integr = np.sum(np.exp(F0_interp_b[Ncl_init_ind : Ncl_B_ind]) * (np.exp(F0_interp_a[Ncl_init_ind : Ncl_B_ind] * Ncl_interp[Ncl_init_ind+1 : Ncl_B_ind+1]) - np.exp(F0_interp_a[Ncl_init_ind : Ncl_B_ind] * Ncl_interp[Ncl_init_ind : Ncl_B_ind])) / (F0_interp_a[Ncl_init_ind : Ncl_B_ind])) * Z
		...
		Ncl_every = np.arange(Ncl_init, Ncl_B)
		t_TST = np.sum(np.exp(F_fnc(Ncl_every))/D_fnc(Ncl_every)) * np.sum(np.exp(-F_fnc(Ncl_every)))
		...
		if(mode == 'FPE_rate_est_exact'):
			if(D_mode == 'const'):
				t_1stpass = scipy.integrate.dblquad(\
					lambda z, y, U_fnc=F0_fnc: np.exp(U_fnc(y) - U_fnc(z)) /  D_fnc(1.0), \
					Ncl_init, Ncl_B, Ncl_min, lambda y: y)
			elif(D_mode == 'surf'):
				t_1stpass = scipy.integrate.dblquad(\
					lambda z, y, U_fnc=F0_fnc, D_fnc=D_fnc: np.exp(U_fnc(y) - U_fnc(z)) /  D_fnc(z), \
					Ncl_init, Ncl_B, Ncl_min, lambda y: y)
		...
		if(to_plot):
			fig_ps, ax_ps, _ = my.get_fig('$N_{cl}$', '$p_s(N_{cl})$')
			fig_psinv, ax_psinv, _ = my.get_fig('$N_{cl}$', '$1/[p_s(N_{cl}) \cdot D(N_{cl})]$')
			
			Ncl_draw = np.linspace(Ncl_min, Ncl_B, 1000)
			y_draw = np.exp(-F_fnc(Ncl_draw))/Z
			ax_ps.plot(Ncl_draw, y_draw, label='inerp')
			minmax = [min(y_draw), max(y_draw)]
			ax_ps.plot([Ncl_init] * 2, minmax, label=r'$N_{init}$')
			ax_ps.plot([Ncl_B] * 2, minmax, label=r'$N_{B}$')
			
			Ncl_draw = np.linspace(Ncl_init, Ncl_B, 1000)
			y_draw = np.exp(F_fnc(Ncl_draw))/D_fnc(Ncl_draw)*Z
			minmax = [min(y_draw), max(y_draw)]
			ax_psinv.plot(Ncl_draw, y_draw, label='inerp')
			ax_psinv.plot([Ncl_init] * 2, minmax, label=r'$N_{init}$')
			ax_psinv.plot([Ncl_B] * 2, minmax, label=r'$N_{B}$')
		...
		Ncl_evol, times, hA = \
			run_dynamics(Ncl_init, dt, Nt, D_fnc, F0_grad_fnc, \
	...
	if(to_plot):
		...
		fig_F0, ax_F0, _ = my.get_fig(r'$N_{cl}$', '$F_0/T$')
		fig_D, ax_D, _ = my.get_fig(r'$N_{cl}$', '$D$ [1/sweep]')
		
		ax_F.errorbar(Ncl_interp, F_data_interp, yerr=d_F_data_interp, label='data')
		ax_F0.errorbar(Ncl_interp, F0_data_interp, yerr=d_F0_data_interp, label='data')
		...
		F0_draw = F0_fnc(Ncl_draw)
		ax_F0.plot(Ncl_draw, F0_draw, label='interp')
		minmax_F0 = [min(F0_draw), max(F0_draw)]
		ax_F0.plot([Ncl_min] * 2, minmax_F0, label=r'$N_{min} = %s$' % (my.f2s(Ncl_min)))
		ax_F0.plot([Ncl_top] * 2, minmax_F0, label=r'$N_{top} = %s$' % (my.f2s(Ncl_top)))
		ax_F0.plot([Ncl_init] * 2, minmax_F0, label=r'$N_{init} = %s$' % (my.f2s(Ncl_init)))
		ax_F0.plot([Ncl_B] * 2, minmax_F0, label=r'$N_{B} = %s$' % (my.f2s(Ncl_B)))
