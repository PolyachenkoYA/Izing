...
def add_data(data, h_min, h_max, T, lbl, i, ax_Nc=None, ax_k=None, to_norm_sgm=0, T_units=False):
	...
	if(ax_Nc is not None):
		...
		ax_Nc.plot(h * T_mult, Nc / T_mult**2, '.-', label=lbl, color=my.get_my_color(i))
		ax_Nc.plot(h * T_mult, Nc_th / T_mult**2, '--', label=None, color=my.get_my_color(i))
...
data = [table_data.qiwei_data, table_data.Nc_reference_data['1.0'], table_data.Nc_reference_data['1.5'], table_data.Nc_reference_data['1.9'], Nc_data_2p0]
N = len(data)

Temps = np.array([1.5, 1.0, 1.5, 1.9, 2.0])
Q_ind = 1
...
fig_Nc, ax_Nc = my.get_fig('$h/J$', r'$N_c$', title=r'$N_c(h)$, $J_c / T = ' + my.f2s(Jc) + '$', xscl='log', yscl='log')
fig_NcNorm, ax_NcNorm = my.get_fig('$h/J$', r'$N_c/\sigma_{T \sim T_c}^2$', title=r'$N_c(h)$, $J_c / T = ' + my.f2s(Jc) + '$', xscl='log', yscl='log')
